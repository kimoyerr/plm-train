...
class CoPE (nn.Module):
    def __init__(self, head_dim, num_heads, npos_max):
        super().__init__()
        self.npos_max = npos_max
        # self.pos_emb = nn.parameter.Parameter(torch.zeros(1, num_heads * head_dim, npos_max))
        self.pos_emb = nn.parameter.Parameter(torch.ones(num_heads, head_dim, npos_max))
...
class Attention(nn.Module):
    ...
    def __init__(self, model_args: ModelArgs):
        ...
        if model_args.pos_emb_type == "cope":
            self.cope = CoPE(self.head_dim, self.n_heads, npos_max)
        else:
            self.cope = None
    ...
    def forward(
        ...
        if self.cope is not None:
            ...
            # attn_logits = torch.bmm(xq, xk.transpose(-1, -2))
            attn_logits = torch.matmul(xq, xk.transpose(-1, -2))
            attn_logits = attn_logits / math.sqrt(self.head_dim)
            # Get causal mask
            L, S = xq.size(-2), xk.size(-2)
            temp_mask = torch.ones(L, S, dtype=torch.int).tril(diagonal=0)
            # Add batch dimension to mask
            temp_mask = temp_mask.unsqueeze(0)
            temp_mask = temp_mask.unsqueeze(0)
            temp_mask = temp_mask.expand(bs, self.n_kv_heads, -1, -1)
            # Push mask to device
            temp_mask = temp_mask.to(x.device)
            attn_logits += temp_mask.log()  # Mask values are log-transformed to convert 1 to 0 and 0 to -inf. This is to make the non-causal tokens invisible to attention
            attn_logits += self.cope(xq, attn_logits)
            attn = F.softmax(attn_logits, dim=-1)
            # output = torch.bmm(attn, xv)
